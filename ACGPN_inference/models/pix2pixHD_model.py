...
def generate_discrete_label(inputs, label_nc, onehot=True, encode=True):
    ...
    if not onehot:
        return label_map.float().cuda() if torch.cuda.is_available(
        ) else label_map.float()
    size = label_map.size()
...
def morpho(mask, iter, bigger=True):
    ...
    for i in range(len(mask)):
        tem = mask[i].cpu().detach().numpy().squeeze().reshape(256, 192,
                                                               1) * 255
    ...
    new = np.stack(new)
    if torch.cuda.is_available():
        return torch.FloatTensor(new).cuda()
...
def encode(label_map, size):
    ...
    if torch.cuda.is_available():
        input_label = torch.cuda.FloatTensor(torch.Size(oneHot_size)).zero_()
        return input_label.scatter_(1, label_map.data.long().cuda(), 1.0)
    else:
        input_label = torch.FloatTensor(torch.Size(oneHot_size)).zero_()
        return input_label.scatter_(1, label_map.data.long(), 1.0)
...
class Pix2PixHDModel(BaseModel):
    ...
    def ger_average_color(self, mask, arms):
        color = torch.zeros(
            arms.shape).cuda() if torch.cuda.is_available() else torch.zeros(
                arms.shape)
    ...
    def encode_input(self, label_map, clothes_mask, all_clothes_label):
        ...
        if torch.cuda.is_available():
            ...
            masked_label = torch.cuda.FloatTensor(
                torch.Size(oneHot_size)).zero_()
            ...
            c_label = c_label.scatter_(1,
                                       all_clothes_label.data.long().cuda(),
                                       1.0)
        else:
            input_label = torch.FloatTensor(torch.Size(oneHot_size)).zero_()
            input_label = input_label.scatter_(1, label_map.data.long(), 1.0)
        ...
        if self.opt.label_nc == 0:
            ...
            if torch.cuda.is_available():
                ...
                input_label = torch.FloatTensor(
                    torch.Size(oneHot_size)).zero_()
                input_label = input_label.scatter_(1, label_map.data.long(),
                                                   1.0)
    ...
    def forward(self, label, pre_clothes_mask, img_fore, clothes_mask, clothes,
        ...
        if torch.cuda.is_available():
            fake_cl_dis = torch.FloatTensor(
                (fake_cl.detach().cpu().numpy() > 0.5).astype(
                    np.float)).cuda()
