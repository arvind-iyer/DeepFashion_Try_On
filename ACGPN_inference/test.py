...
for epoch in range(start_epoch, opt.niter + opt.niter_decay + 1):
    ...
    for i, data in enumerate(dataset, start=epoch_iter):
        ...
        if torch.cuda.is_available():
            ...
            losses, fake_image, real_image, input_label, L1_loss, style_loss, clothes_mask, CE_loss, rgb, alpha = model(
                Variable(data['label']), Variable(data['edge']),
                Variable(img_fore), Variable(mask_clothes),
                Variable(data['color']), Variable(all_clothes_label),
                Variable(data['image']), Variable(data['pose']),
                Variable(data['image']), Variable(mask_fore))
        ...
        if torch.cuda.is_available():
            ...
            a = generate_label_color(
                generate_label_plain(input_label).double())
            b = real_image.double()
            c = fake_image.double()
        d = torch.cat([clothes_mask, clothes_mask, clothes_mask], 1).double()
        combine = torch.cat(
            [a[0], d[0], b[0], c[0], rgb.double()[0]], 2).squeeze()
        # combine=c[0].squeeze()
